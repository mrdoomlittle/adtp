...
void* m(void *__arg_p) {
	...
	if (op == OP_SND) {
		data = 212;
		err = tmp_send(tmp_io, tmp_io_buff(&data, 1), tmp_addr_from_str("2.2.2.2", &err));

		printf("sent: %u\n", data);
		fprintf(stdout, "send_success?: %s.\n", err == TMP_SUCCESS? "yes":"no");
	} else if (op == OP_RCV) {
		data = 0;
		err = tmp_recv(tmp_io, tmp_io_buff(&data, 1), tmp_addr_from_str("1.1.1.1", &err));
...
void holdup(mdl_uint_t __holdup) {usleep(300);}

int main(int __argc, char const *__argv[]) {
	if (*__argv[1] == 's') op = OP_SND;
	if (*__argv[1] == 'r') op = OP_RCV;
	...
	tmp_err_t err;
	if (op == OP_SND)
		tmp_add_iface(&tmp_io, tmp_addr_from_str("1.1.1.1", &err), 0);
	else if (op == OP_RCV)
		tmp_add_iface(&tmp_io, tmp_addr_from_str("2.2.2.2", &err), 0);
	set_pin_state(1, 3);
