...
// the only resion we need cstdint is because we need the uint8_t
# include <cstdint>
...
/* NOTE: we are working with 0 counting to (1 = 0) and (2 = 1) etc */

/* how the clock works :
* if the clock is 0x1/on then tick 1 time any other 0x1/on 
* after that will be ignored same goes with the 0x0/off state
*/

/* this will stop th script from thinking that the first state it a tick
*/
# define def_clock_start_state 0x1

/* what will the pin id of the clock be */
# define def_digit_clock_pin_id 6

/* how many data input pins there are */
# define def_digit_i_pin_count 2

/* this is for debugging but still defualt */
...
namespace adtp { class io_service
{
    ...
    private :
        ...
        uint8_t(* * i_bitset_buffer) = new uint8_t * [ibitset_buff_size];    
        ...
        int unsigned(external_clock_ptcount) = 0; // posative count
        int unsigned(external_clock_ntcount) = 0; // negative count
