...
char const (* * __arg__filter::filter_list_in (char const (* _char_list_in ),
   char const (* _separator_char ), size_t (_char_list_length ) ) )
{
   ...
   for (size_t (list_pos ) = 0; list_pos != nums_add <unsigned int> ( (this-> char_list_length ), 1); list_pos ++)
   {
         ...
         if ( (this-> is_sub_counted ) == true && (this-> got_sub_size ) == true )
         {
            ...
            for (size_t (sub_list_pos ) = 0; sub_list_pos != (this-> sub_counter ); sub_list_pos ++)
               for (size_t (list_pos ) = 0; list_pos != (this-> sub_list_size [sub_list_pos ] ); list_pos ++)
                  (this-> temp_out_list [sub_list_pos ] [list_pos ] ) = (this-> char_list_in [ (nums_add <unsigned int> (list_pos, (this-> sub_start_point [sub_list_pos ] ) ) ) ] );
      ...
      if ( (this-> separator_mapping [list_pos ] ) == true )
      {
         ...
         if ( (this-> is_sub_counted ) == true && (this-> got_sub_size ) == false )
         {
            (this-> sub_start_point [(this-> sub_point_change ) ] ) = nums_add <int> (list_pos, 1);
            for (size_t (sub_list_pos ) = nums_add <int> (list_pos, 1); sub_list_pos != (this-> char_list_length ); sub_list_pos ++)
            {
               if ((this-> separator_mapping [sub_list_pos ] ) == false ) (this-> sub_list_size [(this-> sub_point_change) ] ) ++;
               else break;
            }
...
size_t (__arg__filter::get_sub_counter( ) ) { return (this-> sub_counter ); }
size_t (* __arg__filter::get_sub_list_size( ) ) { return (this-> sub_list_size ); }
